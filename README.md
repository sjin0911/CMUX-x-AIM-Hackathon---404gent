...
- audit 기반 contamination diagnosis: root cause, timeline, node graph
- safe resume prompt와 selective scrub checklist를 만드는 recovery playbook
- `recover --rewrite`로 LLM 기반 safe prompt rewrite 지원
- macOS Keychain, TCC, Gatekeeper quarantine, LaunchAgent persistence, AppleScript automation 감시 룰
- JSONL audit log 기록
...
node src/cli.js diagnose --agent prompt-agent
node src/cli.js recover --agent prompt-agent
...
- `diagnose --agent prompt-agent`: 오염 root cause, timeline, graph, sanitize/resume playbook 출력
- `recover --agent prompt-agent`: safe resume prompt, scrub checklist 출력
- `recover --rewrite`: LLM/mock provider가 위험 의도를 제거한 replacement prompt 제안
...
- `quarantinePane`이 켜져 있으면 오른쪽 split pane에 contamination diagnosis와 recovery playbook이 열립니다.
...
node src/cli.js diagnose --agent demo
node src/cli.js --json diagnose --agent demo
node src/cli.js recover --agent demo
node src/cli.js --config examples/404gent.mock-llm.config.json recover --agent demo --rewrite
node src/cli.js recover --agent demo --apply
```

`recover`는 기본적으로 dry-run입니다. `--rewrite`는 설정된 LLM provider로 안전한 replacement prompt를 만들고, `--apply`는 사람이 scrub checklist를 검토한 뒤 해당 target의 sticky risk state만 reset합니다. audit log는 증거 보존을 위해 삭제하지 않습니다.
