...
404gent는 에이전트 워크플로우에서 위험이 생기는 네 지점을 감시합니다.

1. 에이전트에게 전달되기 전의 프롬프트
2. 실행되기 전의 shell command
3. OS 레벨 파일/프로세스 이벤트
4. 터미널에 출력되기 전의 secret/PII 포함 출력
...
- simulated OS Guard 이벤트로 민감 파일 open, 네트워크 tool exec, 파괴적 exec 감시
...
- Node, rules, audit, state, OS Guard, cmux 상태를 보여줍니다.
...
docs/OS_GUARD.md                   simulated OS Guard demo guide
...
examples/404gent.os-guard.config.json    OS Guard demo config
